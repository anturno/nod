...
const SCHEMA_SNAPSHOT = "f06ee617fddc99e4737549acbf0e2b4d8e6cd460e074405e2f4870aa1012c9f9";

describe("registry", () => {
  test("advertises fx's order plus vision and read_tool_result", () => {
  ...
  test("descriptions say nod, never fx", () => {
    for (const tool of builtinTools) expect(tool.description).not.toMatch(/\bfx\b/);
    expect(lookupTool("subagent")?.description).toContain("nod preserves its trusted base prompt");
  });

  test("metadata matches fx", () => {
...
describe("errors", () => {
  test("error envelopes carry fx's message and suggestion table", () => {
