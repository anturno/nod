...
const globFilesSpec: ToolSpec<globFiles.Input> = {
  ...
  parameters: obj(
    {
      pattern: str("Glob pattern to match, such as src/**/*.zig or *.md."),
...
const grepFilesSpec: ToolSpec<grepFiles.Input> = {
  name: "grep_files",
  description: `Search text files for a literal substring, optionally narrowed by path/include, with output modes for matching lines, files-with-matches, or counts plus head_limit/offset pagination and bounded context_lines for matches mode. ${EXTERNAL} Use include as the type/path filter, such as *.zig. When to use: find exact symbols, strings, TODOs, or usage sites. When NOT to use: regex is not supported; avoid unknown-concept exploration, filename lookup, known-path reads, and shell grep; do not repeat the same or equivalent search after a caller search only finds a definition.`,
  parameters: obj(
    {
