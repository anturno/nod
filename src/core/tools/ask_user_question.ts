...
/** Parsing happens in call so every sentinel reaches the model as a successful output, as in fx. */
export const decode = (args: unknown): DecodeResult<Input> => ok({ args });

// biome-ignore lint/suspicious/noControlCharactersInRegex: fx escapes C0/DEL bytes for terminal safety
