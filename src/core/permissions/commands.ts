 ...
 *
 * ponytail: conservative allowlists over statically-shaped commands replace fx's
 * ~2k-line word-level parser (fx src/core/permissions/command_effect.zig +
 * direct_command.zig). Anything the allowlist does not recognise goes to the
 * reviewer or the prompt, so the ceiling is false negatives (extra reviews),
 * never false positives. Upgrade path: port command_effect.zig's argv planner
 * when the review volume in auto mode hurts.
 */
 ...
 * metacharacters. Mirrors fx permissions.zig isStaticCommand.
 */
