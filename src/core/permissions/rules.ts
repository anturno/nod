/** Persistent permission rules: parsing, matching (fx permissions.zig semantics), and the `nod permissions` snapshot. */
...
/** `*` and `?` glob with backtracking (fx wildcardMatch). */
...
/** Wildcard match where `*`/`?` inside single quotes are literal (fx staticCommandWildcardMatch). */
...
function webFetchDecision(rules: Rule[], target: string): RuleDecision {
  ...
  for (const rule of rules) {
    ...
    // ponytail: fx requires the stored pattern to be canonical already; nod also accepts a bare host.
...
/** The `nod permissions --json` object (fx PermissionsSnapshot.renderJson, plus `source` on each rule). */
...
/** The `nod permissions` text form (fx PermissionsSnapshot.renderText). */
