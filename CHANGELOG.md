...
- Relicensed from MIT to Apache-2.0; `LICENSE` ships in the npm package and release archives.

## [0.1.0] - Unreleased

nod is rebuilt in TypeScript on Bun. Only the ChatGPT (Codex) and Grok subscription backends are kept; the Vercel AI Gateway, `setup`, `teams`, and `credits` are not part of nod.

### Added

- Tools with stable names and schemas: `read_file`, `write_file`, `edit_file`, `glob_files`, `grep_files`, `shell` (background handles, interactive input, stop), `web_fetch`, `web_search`, `vision`, `ask_user_question`, `read_tool_result`, `skill`, `install_skill`, `subagent`, and MCP tool discovery (`capability_search`, `mcp_select_tool`, `mcp_features`).
...
- The CLI surface changed: `nod ask <prompt>` replaces `nod "<prompt>"`, `--provider`/`--yes` are replaced by `nod provider`, `NOD_PROVIDER`, and the permission modes.
