...
- For anything larger than a small fix, open an issue first so we can agree on the approach. nod is written in TypeScript; it stays permission-first, CLI-first, and free of agent frameworks and model SDKs (runtime dependencies are `ink` and `react` only).
